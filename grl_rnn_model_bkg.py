...
class grl_model(object):
    def __init__(self, grl_config, name_scope, num_samples=512, forward_only = False, dtype=tf.float32):
        ...
        self.learning_rate_decay_op = tf.assign(self.learning_rate * grl_config.learning_rate_decay_factor)
        self.dummy_dialogs = []
    ...
    def step(self, session, encoder_inputs, decoder_inputs, target_weights, reward, bucket_id, forward_only):
        ...
        if not forward_only:
            ...
            output_feed = [self.losses[bucket_id], self.encoder_states[bucket_id]]
            for i in xrange(decoder_size):
    ...
    def step_rl(self, session, st_model, bk_model, encoder_inputs, decoder_inputs, target_weights,
        ...
        while True:
            ...
            step_loss, encoder_states, output_logits = self.step(session, encoder_inputs, decoder_inputs, target_weights,
                                                             reward=1, bucket_id=bucket_id, forward_only=False)
        ...
        _, step_loss, _ = self.step(session, init_inputs[0], init_inputs[1], init_inputs[2], bucket_id=init_inputs[3],
